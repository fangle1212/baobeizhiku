  ...
  Editor.prototype = {
    ...
    _setup: function(doc) {
      ...
      if (options.initialContent) {
		options.initialContent = options.initialContent.replace(/\.\.\/\/upload\//g,$G['upload']);
    ...
    getContent: function(cmd, fn, notSetCursor, ignoreBlank, formatter) {
      ...
      return root.toHtml(formatter).replace(new RegExp($G['upload'].replace(/([\(\)\[\]\{\}\^\$\+\-\*\?\.\"\'\|\/\\])/g,"\\$1"),"g"),'..//upload/');
    },
    ...
    getActionUrl: function(action) {
      ...
      if (serverUrl) {
		referer = false;
		if(window.location.href.indexOf('&part=')){
			referer = window.location.href;
		}else if(window.parent.location.href.indexOf('&part=')){
			referer = window.parent.location.href;
		}else if(window.parent.parent.location.href.indexOf('&part=')){
			referer = window.parent.parent.location.href;
		}
        serverUrl = serverUrl + (serverUrl.indexOf("?") == -1 ? "?" : "&") + "action=" + (actionName || "") + (referer? "&referer=" + encodeURIComponent(referer) : '');
