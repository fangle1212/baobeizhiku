    ...
    OnlineFile.prototype = {
				...
				if(folder = li.getAttribute('folder')){
					...
					if (target.tagName.toLowerCase() == 'a') {
						...
						if(domUtils.hasClass(target,'back')){
							...
							_this.initContainer();
							_this.listIndex = editor.getOpt('fileManagerListSize') * (parseInt(target.getAttribute('page'))-1)
							_this.getFileData();
						}
        ...
        getFileData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(editor.getActionUrl(editor.getOpt('fileManagerActionName')+'&folder='+folder+'&type='+editor.getCookie('upfileEasyType')), {
                    timeout: 100000,
                    data: utils.extend({
        ...
        pushData: function (list, total) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    if ( "png|jpg|jpeg|gif|bmp|ico|tif|webp".indexOf(filetype.toLowerCase()) != -1 ) {
                        preview = document.createElement('img');
                        preview.setAttribute('src', urlPrefix + list[i].url.replace('../','../../../../../') + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    } else {
        ...
        getInsertList: function () {
            var i, lis = this.list.children, lt = [], list = [];
