...
(function(){
    ...
    function insertSingle(){
        ...
        editor.execCommand('insertvideo', {
            ...
            height: height.value,
            align: align
        }, 'upload');// isModifyUploadVideo ? 'upload':null);
    }
    ...
    function createPreviewVideo(url){
        if ( !url )return;
        var conUrl = convert_url(url.replace('../','../../../../../'));
        $G("preview").innerHTML = //'<div class="previewMsg"><span>'+lang.urlError+'</span></div>'+
        '<video' +
            ' src="' + conUrl + '"' +
            ' width="' + 420  + '"' +
            ' height="' + 280  + '"' +
            ' loop="true" controls="controls" >' +
        '</video>';
    }
    ...
    OnlineVideo.prototype = {
				...
				if(folder = li.getAttribute('folder')){
					...
					if (target.tagName.toLowerCase() == 'a') {
						...
						if(domUtils.hasClass(target,'back')){
							...
							_this.initContainer();
							_this.listIndex = editor.getOpt('videoManagerListSize') * (parseInt(target.getAttribute('page'))-1)
							_this.getFileData();
						}
        ...
        getFileData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(editor.getActionUrl(editor.getOpt('videoManagerActionName')+'&folder='+folder+'&type='+editor.getCookie('upfileEasyType')), {
                    timeout: 100000,
                    data: utils.extend({
